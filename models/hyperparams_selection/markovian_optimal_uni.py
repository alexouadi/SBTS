...
def get_optimal_order(N, M, K_markov, X, x_past, x_target, N_pi, h, deltati, itter=50):
    ...
    for i in range(len(h)):
        data_sb = np.zeros((len(K_markov), M_test))

        for k in range(len(K_markov)):
            for m in range(M_test):
                data_sb[k, m] = get_last_mark(N, M, K_markov[k], X, x_past[m], N_pi, h[i], deltati, itter)

        mse = np.mean((data_sb - x_target) ** 2, axis=1)
