...
def fake_tensor(generator, x_past, n_lags, **kwargs):
    """
    Helper function that transforms the generated data into dataloader, adapted from different generative models
    Parameters
    ----------
...
def signature(X, degree):
    n_samples, n_features = X.shape
    sig = np.zeros((n_samples, (degree + 1) * n_features))
    ...
    for i in range(n_samples):
        ...
        for d in range(1, degree + 1):
            sig[i, d * n_features: (d + 1) * n_features] = np.cumsum(dx ** d)
