...
def full_evaluation_latest(config, real_data, sbts_data, **kwargs):
    ec = EvaluationComponent(config, real_data, sbts_data, **kwargs)
...
class EvaluationComponent(object):
    ...
    def __init__(self, config, real_data, sbts_data, **kwargs):
        self.config = config
        self.kwargs = kwargs
        ...
        self.metrics_group = {
            'stylized_fact_scores': ['cross_corr', 'cov_loss', 'acf_loss'],
    ...
    def get_data(self, n=1):
        ...
        sample_size_sbts = self.sbts_data.shape[0]
        test_size_sbts = int(sample_size_sbts * self.config.Evaluation.test_ratio)

        idx_all_sbts = torch.randint(self.sbts_data.shape[0], (sample_size_sbts * n,))
        ...
        for i in range(n):
            ...
            idx = idx_all_sbts[i * sample_size_sbts:(i + 1) * sample_size_sbts]
            sbts_train = self.sbts_data[idx[:-test_size_sbts]]
            sbts_test = self.sbts_data[idx[-test_size_sbts:]]
            sbts_train_dl = DataLoader(TensorDataset(
                sbts_train), batch_size=batch_size)
            sbts_test_dl = DataLoader(TensorDataset(
                sbts_test), batch_size=batch_size)

            data.update({i:
                {
                    'real_train_dl': real_train_dl,
                    'real_test_dl': real_test_dl,
                    'fake_train_dl': sbts_train_dl,
                    'fake_test_dl': sbts_test_dl
                }
            })
        return data
                            ...
                            if grp in ['stylized_fact_scores', 'sig_scores', 'distance_based_metrics', 'tail_scores']:
                                ...
                                if metric == 'cexp_loss':
                                    ...
                                    if 'recovery' in self.kwargs and 'supervisor' in self.kwargs:
                                        ...
                                        score = eval_func(real[:1024], combine_dls([fake_train_dl, fake_test_dl]), recovery=recovery,
                                                          supervisor=supervisor)
                                    else:
                                        score = eval_func(real[:1024], combine_dls([fake_train_dl, fake_test_dl]))
